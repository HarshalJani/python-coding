...
class TicTacToe:
  ...
  def mark_position(self, pos, val):
    ...
    if self.grid[r][c] == "":
      ...
      if (r,c) not in self.row[r][val]:
        self.row[r][val].add((r,c))
      if (r,c) not in self.col[c][val]:
  ...
  def check_complete(self):
    ...
    for r in range(len(self.grid)):
      for c in range(len(self.grid[0])):
        ...
        #Check rows
        if len(self.row[r]['X']) == 3:
          return True
        elif len(self.row[r]['O']) == 3:
          return True
        #Check columns
        if len(self.col[c]['X']) == 3:
          return True
        elif len(self.col[c]['O']) == 3:
          return True
        #Check diagnoals
        key = 0 if r == c else 1
        if len(self.diag[key]['X']) == 3:
          return True
        elif len(self.diag[key]['O']) == 3:
          return True
